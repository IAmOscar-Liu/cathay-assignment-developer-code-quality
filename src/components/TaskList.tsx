...
export const TaskList: React.FC = () => {
  const { tasks, deleteTask, updateTask, filterTasks } = useTasks();
  ...
  const filteredTasks = useMemo(() => {
    ...
    // );
    // 'filterTasks' is already in 'useTasks()'
    return filterTasks(filter.status, filter.priority);
  }, [tasks, filter]);
  ...
  const renderTaskActions = (task: Task) => {
    const handleDelete = () => {
      ...
      //   deleteTask(task.id);
      // }
      // put confirm window in 'deleteTask'
      deleteTask(task.id);
    };
